...
- Previous InterviewRoundV2 assessment, recovery state, exact current heads, and host-supplied allowed content hashes.
...
- `goal.evidence`, `constraints.evidence`, `success.evidence`, optional `context.evidence`, deferral `evidence`, and readiness `evidenceIds` contain **exact saved answer IDs**, not source hashes, prose explanations, filenames, or invented IDs.
...
9. After each answer or research return, call `solar_interview_round` once with the complete InterviewRoundV2 required by the runtime tool schema. Assess only the current heads and use exact host-supplied hashes; do not add head fields or invent fields, IDs, hashes, answers, or research.
...
- On V2 validation failure, repair the complete report against the same saved heads. Do not ask another question to hide a formatting error.
...
- On re-entry, reload saved answers, corrections, heads, material state, recovery attempts, and deferrals. Continue from the largest current gap instead of relying on transcript memory.
