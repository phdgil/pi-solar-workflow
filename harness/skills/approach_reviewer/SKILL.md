...
4. For **software**, inspect architecture fit and the seams: existing module/API/tool support in provenance, signatures and formats across callers, path/cwd/runtime assumptions, data ownership, dependency order, side effects, error handling, and whether the exact command can exercise the named interface. Flag guessed or incompatible interfaces.
   - The host tool inventory lists Pi APIs, not programs executed through a shell tool. An interpreter's absence from that list is not evidence it is missing.
   - Distinguish a concrete unsupported interface or known unavailable prerequisite from an explicitly stated normal runtime assumption. If the permitted acceptance command already fails closed on a missing prerequisite, that hypothetical failure alone does not require another probe or a material finding.
   - Never recommend commands beyond the supplied authority as an ordinary plan repair. When a feasibility claim overstates evidence, prefer accurately stating the assumption and its existing failure gate; if extra access is genuinely indispensable, require an explicit blocked/permission boundary rather than inventing a grant.
...
7. Put the strongest defect first. Use reviewer-prefixed unique IDs such as `AR-interface-format`. Each finding names the exact plan location, concrete mismatch, and smallest required correction. `material` means a concrete method defect, unsupported interface, or consequential failure that the plan does not responsibly handle; the mere possibility of an already contained operational failure is not sufficient. Style or optional hardening is `advisory`.
