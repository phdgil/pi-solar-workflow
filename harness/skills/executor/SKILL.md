...
2. Resolve each step capability to its exact tool and allowed canonical path or exact command. Pass file-tool paths verbatim as declared: workspace-relative with forward slashes, never absolute paths, drive or ADS syntax, even when they refer to the same file. Do not add flags, substitute tools, broaden paths, install dependencies, publish, commit, or touch an external system unless that exact authority is declared.
...
5. Inspect the resulting content, not only its path, using only read or command capabilities declared for this current step. An output declaration or paths listed under a command do not authorize a standalone read. Check the step's stated invariants and observable acceptance properties such as schema, counts, ordering, duplicates, boundary values, and error behavior when applicable.
